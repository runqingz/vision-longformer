...
class BasicAABlock(nn.Module):
    ...
    def __init__(
        ...
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        if groups != 1 or base_width != 64:
            raise ValueError("BasicAABlock only supports groups=1 and base_width=64")
        if dilation > 1:
            raise NotImplementedError("Dilation > 1 not supported in BasicBlock")
        # Both self.conv1 and self.downsample layers downsample the input when stride != 1
        self.conv1 = conv3x3(inplanes, planes, stride)
        ...
        #Layer Norm
        self.attn_norm = attn_norm_layer
        #Projection layer of concat output
        self.projection = conv1x1(attn_planes + planes, planes)
